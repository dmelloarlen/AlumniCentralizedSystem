...
export default function InternshipsJobs() {
    ...
    const filterInternshipsJobs = (filterItem) =>{
       return internshipsJobs.filter(job => job.title.toLowerCase().includes(filterItem.toLowerCase()) || job.company.toLowerCase().includes(filterItem.toLowerCase()));
    };

  return (
    <div>
        <h1 className='text-2xl font-medium text-center mt-10'>Internships / Jobs</h1>
      ...
      <section className="mt-8 display flex gap-6 flex-wrap justify-center">
              {filterInternshipsJobs(filterItem).length > 0 ? (
                filterInternshipsJobs(filterItem).map((job, i) => (
                  <div key={i} className="bg-white rounded-lg p-6 mb-4 hover:scale-105 transition-transform shadow-xl duration-300 w-80">
                    <div className="flex items-center justify-end mb-4 cursor-pointer" onClick={() => navigate('/profile')}>
                      <img
                      ...
                      />
                      {job.ref && <span className="text-xs text-gray-500 ms-2">{job.ref}</span>}
                    </div>
                    <div >
                ...
                <p className="text-gray-600 text-center w-full">No internships or jobs found matching your search criteria.</p>
              )}
            </section>
