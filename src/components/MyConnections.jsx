...
export default function MyConnections() {
  ...
  return (
    <div className='flex justify-center'>
        <div>
            {sampleData.map((data) =>(
                <div className='container w-sm lg:w-6xl my-10 p-2 flex justify-between gap-2 shadow-lg rounded-lg hover:scale-102 transition-transform duration-300 cursor-pointer' onClick={()=> navigate('/profile')}>
                  ...
                  <div className='mt-4'>
                    <CiChat1 className='text-2xl cursor-pointer' onClick={(e)=> {e.stopPropagation(); navigate('/chat')}}/>
                  </div>
                </div>
              ))}
        </div>
